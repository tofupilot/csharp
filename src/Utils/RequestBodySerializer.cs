    ...
    internal class RequestBodySerializer
    {
        ...
        )
        {
            ...
            if (Utilities.IsClass(request))
            {
                ...
                if (prop != null)
                {
                    var metadata = prop.GetCustomAttribute<ApiMetadata>()?.GetRequestMetadata();
                    if (metadata != null)
                    {
        ...
        )
        {
            ...
            if (Utilities.IsClass(request))
            {
                ...
                foreach (var prop in props)
                {
                    ...
                    var metadata = prop.GetCustomAttribute<ApiMetadata>()?.GetFormMetadata();
                    if (metadata == null)
                    {
                        continue;
                    }
        ...
        private static HttpContent SerializeMultipart(object request, string mediaType)
        {
            ...
            foreach (var prop in properties)
            {
                ...
                var metadata = prop.GetCustomAttribute<ApiMetadata>()?.GetMultipartFormMetadata();
                if (metadata == null)
                {
                    continue;
                }
        ...
        private static (string fileName, byte[] content) ExtractFileProperties(PropertyInfo[] fileProps, object fileObject)
        {
            ...
            foreach (var fileProp in fileProps)
            {
                var fileMetadata = fileProp
                    .GetCustomAttribute<ApiMetadata>()
